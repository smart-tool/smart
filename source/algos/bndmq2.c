...
/*
 * Backward Nondeterministic DAWG Matching using q-grams designed for large
 * patterns The present implementation searches for prefixes of the pattern of
 * length 32. When an occurrence is found the algorithm tests for the whole
      ...
      do {
        if (D >= M) {
          if (j > first)
            ...
            if (first <= n - p_len && !memcmp(x, y + first, p_len))
              OUTPUT(first);
          }
        }
        j = j - 1;
