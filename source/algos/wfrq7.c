...
#define Q 7
#define HASH(j)                                                                \
  (y[j] << 12) + (y[j - 1] << 10) + (y[j - 2] << 8) + (y[j - 3] << 6) +        \
      (y[j - 4] << 4) + (y[j - 5] << 2) + y[j - 6]
...
void preprocessingQ(unsigned char *x, int m, char *F) {
  ...
  for (i = Q - 1; i < m; i++) {
    ...
    h = (x[i] << 12) + (x[i - 1] << 10) + (x[i - 2] << 8) + (x[i - 3] << 6) +
        (x[i - 4] << 4) + (x[i - 5] << 2) + x[i - 6];
    F[h] = TRUE;
    for (j = i - Q; j >= stop; j -= Q) {
      h = (h << 14) + (x[j] << 12) + (x[j - 1] << 10) + (x[j - 2] << 8) +
          (x[j - 3] << 6) + (x[j - 4] << 4) + (x[j - 5] << 2) + x[j - 6];
      F[h] = TRUE;
    }
  }
}
