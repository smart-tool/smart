 ...
 * Fast-Search Algorithms: New Efficient Variants of the Boyer-Moore
 * Pattern-Matching Algorithm. J. Autom. Lang. Comb., vol.10, n.5/6,
 * pp.589--608, (2005).
 */
...
void Forward_Suffix_Function(unsigned char *x, int m, int bm_gs[XSIZE][SIGMA],
  ...
  for (suffix_len = 0; suffix_len <= m; suffix_len++) {
    ...
    if (bm_gs[m - suffix_len][x[0]] > m)
      bm_gs[m - suffix_len][x[0]] = m;
    temx[last] = -1;
  }
}
