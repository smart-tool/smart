...
// Another important structures are automata and specifically suffix automata
// (see chapter 22). Basically automata are directed graphs. We will use the
// following interface to manipulate automata:
/*
...
int getTarget(Graph g, int v, unsigned char c) {
  // if (g != NULL && g->target != NULL && v < g->vertexNumber && v*c <
  // g->edgeNumber)
...
/* returns the shift of the edge from vertex v
   labelled by character c in graph g */
int getShift(Graph g, int v, unsigned char c) {
  // if (g != NULL && g->shift != NULL && v < g->vertexNumber && v*c <
  // g->edgeNumber)
