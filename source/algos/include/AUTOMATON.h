...
/*void buildSuffixAutomaton(unsigned char *x, int m,
   int *ttrans, int *tlength, int *tsuffix, unsigned char *tterminal, int
*tshift, int *position) { int i, art, init, last, p, q, r, counter,j, source,
      ...
      if (getTarget(p, c) == UNDEFINED) {
         ...
         if (getLength(p) + 1 == getLength(getTarget(p, c))) {
            ...
            //copyVertex(r, getTarget(p, c));
                        ...
                        for(j=0; j<SIGMA; j++) setTarget(target, j,
getTarget(source,j) ); for(j=0; j<SIGMA; j++) setShift(target, j,
getShift(source,j) ); tterminal[target] = tterminal[source]; tsuffix[target] =
tsuffix[source]; tlength[target] = tlength[source]; position[target] =
position[source];

            memcpy(ttrans+r*SIGMA, ttrans+getTarget(p, c)*SIGMA,
SIGMA*sizeof(int)); setLength(r, getLength(p) + 1); setSuffixLink(r,
getSuffixLink(getTarget(p, c))); setSuffixLink(getTarget(p, c), r);
            setSuffixLink(q, r);
            while (p != art && getLength(getTarget(p, c)) >= getLength(r)) {
               setShift(p, c, getPosition(getTarget(p, c)) - getPosition(p) -
1); setTarget(p, c, r); p = getSuffixLink(p);
            }
         }
      last = q;
   }
...
void buildSimpleSuffixAutomaton(unsigned char *x, int m, int *ttrans,
                                int *tlength, int *tsuffix,
